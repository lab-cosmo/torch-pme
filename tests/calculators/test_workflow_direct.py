...
# MADELUNG_CSCL = torch.tensor(2 * 1.7626 / math.sqrt(3)) # periodic case
MADELUNG_CSCL = torch.tensor(2 * math.sqrt(3))
...
def test_all_types():
    descriptor = DirectPotential(all_types=[8, 55, 17])
...
def test_all_types_error():
    descriptor = DirectPotential(all_types=[17])
...
def test_dtype_device():
    ...
    DP = DirectPotential()
    potential = DP.compute(types=types, positions=positions)
...
def test_inconsistent_device_charges():
    ...
    with pytest.raises(ValueError, match=match):
        DP.compute(types=types, positions=positions, charges=charges)


def test_inconsistent_dtype_charges():
    """Test if the charges and positions have inconsistent dtype and error is raised."""
