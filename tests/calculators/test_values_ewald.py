...
@pytest.mark.parametrize("scaling_factor", [1 / 2.0353610, 1.0, 3.4951291])
def test_parameter_choose(crystal_name, scaling_factor, calc_name):
    """
    Check that the Madelung constants obtained from the Ewald sum calculator matches
    the reference values.
    In this test, only the charge-neutral crystal systems are chosen for which the
    ...
    if calc_name == "ewald":
        atol = 1e-3
        calc, sr_cutoff = EwaldPotential.from_accuracy(atol, pos, charges, cell)
        rtol = 4e-6
    elif calc_name == "pme":
        pass
    ...
    torch.testing.assert_close(madelung, madelung_ref, atol=atol, rtol=rtol)
