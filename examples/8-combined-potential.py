...
ax.plot(dist, pot_1.lr_from_kvectors(k), label="p=1")
ax.plot(dist, pot_2.lr_from_kvectors(k), label="p=2")

ax.plot(
    dist, potential.lr_from_kvectors(k).detach(), label="Combined potential", c="black"
)
ax.plot(
    dist,
    pot_1.lr_from_kvectors(k) + pot_2.lr_from_kvectors(k),
    label="Explict combination",
    ls=":",
)
