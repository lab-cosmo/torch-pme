...
class Mesh:
    ...
    def __init__(
        ...
        if (((box - torch.eye(3) * mesh_size) ** 2)).sum() > 1e-8:
            raise ValueError(
                "The current implementation is restricted to cubic boxes. "
            )
        self.box_size = mesh_size

        # Computes mesh parameters
        # makes sure mesh size is even, torch.fft is very slow otherwise (possibly
        # needs powers of 2...)
        ...
        if self.mesh_style == "real_space":
            ...
            self.values = torch.zeros(
                size=(n_channels, n_mesh, n_mesh, n_mesh), device=device, dtype=dtype
            )
            ...
            self.values = torch.zeros(
                size=(n_channels, n_mesh, n_mesh, len(self.grid_z)),
                device=device,
                dtype=dtype,
            )
        else:
...
class FieldBuilder(torch.nn.Module):
    ...
    def compute(
        self, system: System, embeddings: Optional[torch.tensor] = None
    ) -> Mesh:
        device = system.positions.device

        # If atom embeddings are not given, build them as one-hot encodings of
        # the atom types
        if embeddings is None:
            all_species, species_indices = torch.unique(
                system.species, sorted=True, return_inverse=True
            )
            embeddings = torch.zeros(
                size=(len(system.species), len(all_species)), device=device
            )
            embeddings[range(len(embeddings)), species_indices] = 1.0

        if embeddings.shape[0] != len(system.species):
            raise ValueError(
                f"The atomic embeddings length {embeddings.shape[0]} does not match "
                f"the number of atoms {len(system.species)}."
            )
            ...
            if order == 2:
                ...
                return torch.stack(
                    [
                        1
                        / 48
                        * (1 - 6 * dist + 12 * dist * dist - 8 * dist * dist * dist),
                        1
                        / 48
                        * (23 - 30 * dist - 12 * dist * dist + 24 * dist * dist * dist),
                        1
                        / 48
                        * (23 + 30 * dist - 12 * dist * dist - 24 * dist * dist * dist),
                        1
        ...
        def interpolate(mesh, positions_cell, embeddings):
            ...
            rp_shift = torch.stack(
                [
                    (positions_cell_idx + i) % mesh.n_mesh
                    for i in range(
                        1 - (self.mesh_interpolation_order + 1) // 2,
                        1 + self.mesh_interpolation_order // 2,
                    )
                ],
                dim=0,
            )
...
class MeshInterpolator(torch.nn.Module):
    ...
    def compute(self, mesh: Mesh, points: torch.tensor):
        ...
        rp_shift = torch.stack(
            [
                (points_cell_idx - 1 + mesh.n_mesh) % mesh.n_mesh,
                (points_cell_idx + 0) % mesh.n_mesh,
                (points_cell_idx + 1) % mesh.n_mesh,
            ],
            dim=0,
        )
        """
        ...
        """
        interpolated_values = torch.zeros(
            (points.shape[0], mesh.n_channels), dtype=points.dtype, device=points.device
        )
        if self.mesh_interpolation_order == 3:
            ...
            # now compute the product of weights with the mesh points, using index
            # unrolling to make it quick this builds indices corresponding to three
            # nested loops
            x_shifts, y_shifts, z_shifts = torch.meshgrid(
            ...
            interpolated_values = (
                (
                    mesh.values[:, x_indices, y_indices, z_indices]
                    * weight[x_shifts, :, 0]
                    * weight[y_shifts, :, 1]
                    * weight[z_shifts, :, 2]
                )
