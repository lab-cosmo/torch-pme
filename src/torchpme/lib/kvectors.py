...
def _generate_kvectors(
    ns: torch.Tensor, cell: torch.Tensor, squeezed: bool
) -> torch.Tensor:
...
def generate_kvectors_for_mesh(ns: torch.Tensor, cell: torch.Tensor) -> torch.Tensor:
    ...
    """
    return _generate_kvectors(ns=ns, cell=cell, squeezed=False)
...
def generate_kvectors_squeezed(ns: torch.Tensor, cell: torch.Tensor) -> torch.Tensor:
    ...
    """
    return _generate_kvectors(ns=ns, cell=cell, squeezed=True).reshape(-1, 3)
