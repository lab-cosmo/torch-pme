...
class CubicSpline(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        interpolated = (
            a * (self.y_points[i] + (a*a-1)* self.d2y_points[i]* h2over6) +
...
class CubicSplineLongRange(CubicSpline):
    ...
    def __init__(self, x_points:torch.Tensor, y_points:torch.Tensor):
        ...
        ix_points = torch.cat([torch.zeros((1,), dtype=x_points.dtype, device=x_points.device), torch.reciprocal(x_points[::-1])], dim=0)
        iy_points = torch.cat([torch.zeros((1,), dtype=x_points.dtype, device=x_points.device), y_points[::-1]], dim=0)
...
def compute_spline_ft(k_points:torch.Tensor, x_points:torch.Tensor, y_points:torch.Tensor, d2y_points:torch.Tensor):
    r"""
