...
class InversePowerLawPotential(RangeSeparatedPotential):
    ...
    :param exclude_radius: TODO document
    """
    ...
    def __init__(
        ...
        exclude_radius: Union[float, torch.Tensor] = 0.0,
    ):
        super().__init__()
    ...
    def f_cutoff(self, dist: torch.Tensor) -> torch.Tensor:
        return 0.5 * (torch.cos(torch.pi * dist / self.exclude_radius) + 1)
    ...
    def from_dist(self, dist: torch.Tensor) -> torch.Tensor:
        ...
        :param dist: torch.tensor containing the distances at which the potential is to
            be evaluated.
        """
        if self.exclude_radius == 0.0:
            return torch.pow(dist, -self.exponent)
        else:
            return torch.where(
            dist < self.exclude_radius,
            -self.lr_from_dist(dist)*self.f_cutoff(dist),
            torch.pow(dist, -self.exponent)
        )
    ...
    def sr_from_dist(self, dist: torch.Tensor) -> torch.Tensor:
        ...
        if self.exclude_radius == 0.0:
            ...
            val = -self.lr_from_dist(dist)*self.f_cutoff(dist)
            return val
...
class RemoveInterior(RangeSeparatedPotential):
    ...
    def __init__(
        ...
        self.exponent :float = self._potential.exponent
        self.smearing :float = self._potential.smearing
...
class InversePowerLawPotentialNoInterior(InversePowerLawPotential):

    def __init__(self, cutoff: Union[float, torch.Tensor], **kwargs):
        InversePowerLawPotential.__init__(self, **kwargs)
        self.cutoff = cutoff
    ...
    def sr_from_dist(self, dist: torch.Tensor) -> torch.Tensor:        
        pots = -InversePowerLawPotential.lr_from_dist(self,dist) 
        return pots* self.f_cutoff(dist)
...
class NoInteriorMixIn(RangeSeparatedPotential):
    ...
    def __init__(self, cutoff: Union[float, torch.Tensor], **kwargs):
        ...
        self.cutoff = cutoff
        self.base_potential : RangeSeparatedPotential = self.__class__.__bases__[1]

    def from_dist(self, dist: torch.Tensor) -> torch.Tensor:
        return torch.where(
            dist < self.cutoff,
            self.base_potential.from_dist(self,dist)
            - self.base_potential.sr_from_dist(self, dist)
            + self.sr_from_dist(dist),
            self.base_potential.from_dist(self,dist),
        )

    def sr_from_dist(self, dist: torch.Tensor) -> torch.Tensor:
        return -self.base_potential.lr_from_dist(self,dist) * self.f_cutoff(dist)
